...
knitr::opts_chunk$set(include = TRUE, eval = TRUE, echo = TRUE, message = FALSE, warning = FALSE)
```
...
```
This plot is bimodal and skewed right.  The mean is around $90,000 and the standard
deviation is around $50,000.
```
...
```
No, the mean is not a good statistic, because the data is very skewed.
The mean gets "pulled" out towards the extreme values in the tail.
```
...
```
Yes!
The original data is not Normally distributed at all, but the SAMPLE MEAN is
still approximately Normal, according to the CLT.
```
...
```{r}
my_boots <- SF_Salaries %>% modelr::bootstrap(500)

my_boots <- my_boots %>%
  mutate(
    sample_means = map_dbl(strap, ~mean(data.frame(.x)$TotalPay))
  )
...
my_boots <- my_boots %>%
  mutate(
    midhinge = map_dbl(strap, ~midhinge(data.frame(.x)$TotalPay)),
    trim_5 = map_dbl(strap, ~trimmed_mean(data.frame(.x)$TotalPay, 0.05)),
    trim_10 = map_dbl(strap, ~trimmed_mean(data.frame(.x)$TotalPay, 0.10)),
    trim_25 = map_dbl(strap, ~trimmed_mean(data.frame(.x)$TotalPay, 0.25)),
    median = map_dbl(strap, ~median(data.frame(.x)$TotalPay))
  )


my_stats <- my_boots %>% 
  pivot_longer(sample_means:median, 
               names_to = "Statistic", 
               values_to = "Value") 
...
cis <- my_stats %>%
  group_by(Statistic) %>%
  summarize(lower_ci = quantile(Value, 0.025),
...
my_stats %>%
  ...
  geom_vline(aes(xintercept = lower_ci), color = "red") +
  geom_vline(aes(xintercept = upper_ci), color = "red") +
...
```
No single right answer - all the statistics but the mean are resistant to 
extreme values.
```
